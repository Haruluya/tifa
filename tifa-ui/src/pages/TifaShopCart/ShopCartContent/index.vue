<template>
    ...
    <div class="shopCartContent">
        <div class="shopCartTitle">TIFA-全部商品 {{cartList.length}}</div>
            ...
            <div class="summit" v-if="showButton">
   ...
   data() {
    ...
    computed:{
      ...mapGetters(['cartList']), 
        sumPrice(){
            let sumPrice = 0;
            this.cartList.productInfo.forEach((item,index) => {
                sumPrice += item.promoteprice * this.cartList.numList[index];
            });
