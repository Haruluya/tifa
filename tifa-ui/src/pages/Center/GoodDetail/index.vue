            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  class="itxt"
                  v-model="goodAmount"
                  @change="validateGoodAmount"
                />
                <a href="javascript:" class="plus" @click="goodAmount++">+</a>
                <a
                  href="javascript:"
                  class="mins"
                  @click="goodAmount > 1 ? goodAmount-- : (goodAmount = 1)"
                  >-</a
                >
              </div>
              <div class="add">
...
export default {
  ...
  mounted() {
    this.$store.dispatch("getGoodData", this.$route.params.goodId);
  },
  ...
  methods: {
    ...
    //验证商品数量表单。(防止用户输入有问题)
    //非数字或<1取1.
    validateGoodAmount(event) {
      let amount = event.target.value * 1;
      if (isNaN(amount) || amount < 1) {
        this.goodAmount = 1;
      } else {
        this.goodAmount = parseInt(value);
      }
    },
    ...
    async addShopcar() {
      try {
        await this.$store.dispatch("updateShopCart", {
            ...
            goodAmount: this.goodAmount,
        });
        // sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo));
        // this.$router.push({name:'addcartsuccess',query:{goodAmount:this.goodAmount}});
